...
context window can hold, so they store some of it externally. The
question is what to keep. Today's memory systems---Mem0, Letta, Mastra,
Zep, our own KGmem---answer that question with hand-written code. The
store grows from use; the control layer that picks what to commit
typically does not.

We frame memory as a budgeted decision problem. Given a hard retention
budget $K$, the write policy emits a sequence of typed operations
(\texttt{lookup\_entity}, \texttt{create\_entity},
\texttt{update\_state}, \texttt{merge\_entities},
\texttt{add\_relation}, \texttt{mark\_contradiction}, \texttt{forget},
\texttt{noop}) into a typed-transition knowledge graph. The read side
is held fixed: a hand-coded hybrid retriever fusing BM25 over entity
text, dense embedding similarity, and named-entity lookup via
reciprocal-rank fusion. A controlled audit on a paired 50-question
LoCoMo sample showed this stack recovers roughly 75\% of the accuracy
gap that pure entity-only retrieval introduces, so observed gains
from the learned write policy are not artefacts of an underpowered
reader.

The technical contribution is the dense write reward.
Trajectory-level signals (final-answer judge, random-$K$ margins,
dia\_id-overlap with what the reader retrieves from full text) all
yield one scalar per write trajectory that GRPO must then diffuse
over a 4--12-token tool-call sequence; per-op resolution is lost.
We propose \emph{per-op counterfactual marginal utility}: for each
mutating write op $a_i$, we replay the trajectory minus $a_i$ and
measure the per-question judge delta the omission produces against a
held-out QA battery. The trajectory reward is the sum of per-op
deltas, but each op carries its own credit signal at GRPO advantage
time. Lookups and noops are skipped (their effect on the post-write
graph is zero by construction). The result is a dense, per-op reward
at roughly $5\times$ the cost of a trajectory-level signal. We blend
this with the reader's absolute judge score as an anchor
($w_{\text{cf}} = 0.7$, $w_{\text{qa}} = 0.3$).

The success criterion is strict. A learned write policy must beat
\emph{random subsampling at matched retention budget} on LoCoMo and
LongMemEval-S; this is the content-agnostic efficiency frontier any
contribution has to clear. It must also beat a BM25-based heuristic
write policy that selects turns by query overlap. \TBD{Report headline
numbers once the long Phase~B run lands. Anchor: random subsampling at
$F = 0.1$ on LoCoMo achieves $\sim 4\%$ judge-graded QA, BM25 heuristic
at the same $F$ achieves $\sim 63\%$ on LongMemEval-S; the learned
policy needs to land above the latter.}

The framing we want to leave the reader with: facts live in the store,
strategy lives in the weights. A single trained adapter can run
against any user's local store, with no per-user fine-tuning and no
...
\begin{enumerate}
  \item A reframing of LLM memory as a \emph{budgeted decision
    problem}: under a hard retention budget $K$, the learnable layer
    is the write policy that picks which $K$ items to commit. Read
    becomes a hand-coded hybrid retrieval stack that the write policy
    optimises against.
  \item \textbf{Per-op counterfactual marginal utility} as the dense
    reward for memory-write GRPO. Each mutating op gets credit equal
    to the per-question judge delta its omission would cause, summed
    over a held-out battery. This is the COMA-style attribution
    \cite{foerster2018coma} applied at the op granularity our typed
    op vocabulary makes natural, and it gives GRPO per-op resolution
    that trajectory-level rewards lose to uniform diffusion.
  \item A backend-retriever-aligned evaluation protocol that controls
    for the dominant confound in memory-system evaluation: an audit on
    a paired LoCoMo sample shows mismatched retrievers can swing QA
    accuracy by $0.40$ in absolute terms (McNemar $p = 1.9 \times
    10^{-6}$). Our read-side retriever fuses BM25, dense, and NER via
    RRF, and we run training and evaluation on the same retriever.
  \item Two clean baselines the learned policy must beat: random
    subsampling at matched $F$ (the efficiency frontier; trivial to
    compute, hard to dominate at low $F$) and a BM25-based heuristic
    write policy. We report the area under the efficiency curve
    (AUEC) as the headline metric, with statistical significance via
    paired McNemar tests stratified by question category.
  \item Empirical results on LoCoMo and LongMemEval-S, plus a
    head-to-head against KGmem's hand-coded extraction pipeline on
    personal ChatGPT-export data with self-generated questions.
    \TBD{Numbers fill in once the long Phase~B run completes.}
\end{enumerate}
...
\paragraph{Reward.} Let $\tau = (a_1, a_2, \dots, a_n)$ be the
sequence of ops the policy emitted in this episode, and let
$\mathcal{M}(\tau) \subseteq \{1,\dots,n\}$ index the
\emph{mutating} ops (every op except \texttt{lookup\_*} and
\texttt{noop}, which by construction leave the post-W graph
unchanged). Write $M_\tau$ for the post-budget memory state when all
of $\tau$ is replayed and $M_{\tau\setminus i}$ for the state when
$\tau$ is replayed without $a_i$. The per-op marginal utility is
\begin{equation}
...
\end{equation}
The trajectory reward GRPO sees is then
\begin{equation}
  r_W(\tau) \;=\; w_{\text{cf}} \!\!
    \sum_{i \in \mathcal{M}(\tau)} \!\! \Delta_i(\tau, Q)
  \;+\; w_{\text{qa}} \cdot \frac{1}{|Q|} \sum_{q \in Q}
      \mathrm{judge}\!\bigl(R(q, M_\tau), \mathrm{gold}_q\bigr)
  \;-\; \lambda_W \, c_W(\tau).
\end{equation}
We default to $w_{\text{cf}} = 0.7$, $w_{\text{qa}} = 0.3$,
$\lambda_W = 0.001$, $K_{\max} = 12$. The QA term anchors the policy
to the actual downstream task; the per-op term provides the
attribution. Op cost $c_W(\tau)$ is small because the hard budget
already does most of the regularising; we report it for diagnostic
logging more than for reward shaping.

\paragraph{Why per-op counterfactual.} Trajectory-level rewards
(final-answer judge, random-$K$ margins, dia\_id-overlap with what the
reader retrieves from full text) all yield one scalar per write
trajectory. GRPO uses that scalar as the trajectory advantage and
diffuses it uniformly over the 4--12 emitted tokens. The signal-to-
noise ratio per op is poor; entropy collapse and slow convergence
follow. Per-op counterfactual replaces the diffused signal with a
sum of per-op deltas, each computable with one extra reader+judge
pass per mutating op per question. At typical per-trajectory budgets
($|\mathcal{M}| \approx 2$, $|Q| \approx 4$) this is roughly
$5\times$ the cost of a trajectory-level signal but gives GRPO the
per-op resolution it needs. The technique is the analogue of the
COMA-style counterfactual baseline from cooperative multi-agent RL
\cite{foerster2018coma}, applied at the op granularity that our typed
op vocabulary makes natural. Concurrent memory-RL work
(Memory-R1~\cite{memoryR1}, DeltaMem~\cite{deltamem},
Mem-$\alpha$~\cite{memalpha}) trains memory managers with composite
trajectory-level rewards; per-op counterfactual attribution is, to
our knowledge, novel in this setting.

\paragraph{What we deprecated.} Earlier drafts of this paper used
\emph{evidence coverage} (fraction of a question's gold dia\_ids
preserved) and then \emph{reader-overlap} (jaccard of dia\_ids the
reader retrieves from full text vs.\ those preserved in the KG). An
audit found that gold coverage correlates with the trivial
turns-kept feature at Spearman $\rho \approx 0.98$, making it
non-content-sensitive. Reader-overlap was less brittle but
structurally bounded low because the reader retrieves chunks of
$\sim$6 dia\_ids each from full text whereas the post-W graph
stores entities tagged with single dia\_ids. Both signals are kept
as logged diagnostics for ablation comparison; the v3 reward sets
their weights to 0.
