...
budget $K$, the write policy decides which $K$ items from a
conversation to commit to a knowledge graph, and is rewarded if a
frozen reader can answer downstream questions from those $K$ items.
The reward signal is \emph{reader-overlap}: for each question, take
what a frozen reader retrieves from the full conversation text---the
set of dialogue turns the reader believes are relevant---and score how
many of those turns the post-write knowledge graph actually
preserved. No per-question evidence annotation is required; in
controlled studies this signal correlates with gold evidence coverage
at within-turn Spearman $\rho \approx 0.61$, while gold coverage
itself correlates almost perfectly ($\rho \approx 0.98$) with the
trivial number-of-turns-kept feature, making it a misleading reward in
isolation.

We train a LoRA adapter on a 4B-parameter open-source model with GRPO,
where each episode is one conversation turn, the write policy emits up
to four tool-call operations into a typed knowledge graph, and the
reward blends reader-overlap ($w = 0.7$) with the reader's
LLM-judge-scored answer accuracy on a held-out battery ($w = 0.3$).
The reader is held fixed: a hand-coded composite of BM25 over entity
text, dense embedding similarity, and named-entity lookup, fused via
reciprocal-rank fusion (RRF). An audit on a paired 50-question LoCoMo
sample showed this hybrid retriever recovers roughly 75\% of the
accuracy gap that pure entity-only retrieval introduces, making it
strong enough that gains from the learned write policy are not
artefacts of an underpowered read backend.
...
\begin{enumerate}
  \item A reframing of LLM memory as a \emph{budgeted decision
    ...
    optimises against.
  \item A label-free dense reward, \emph{reader-overlap}, that
    requires no per-question evidence annotations and is robust to the
    $n_{\text{kept}}$ confound that defeats raw evidence-coverage. We
    show coverage by itself correlates with the trivial
    turns-kept feature at $\rho \approx 0.98$, making the budgeted
...
\paragraph{Reward.} The reward blends a label-free \emph{reader-overlap}
signal with a judged QA term, minus a residual op-cost regulariser:
\begin{align}
  r_W(\tau) \;=\; w_{\text{ov}} \cdot
    \mathrm{overlap}(R, F, M_\tau, Q)
  + w_{\text{qa}} \cdot \frac{1}{|Q|} \sum_{q \in Q}
      \mathrm{judge}\bigl(R(q, M_\tau), \mathrm{gold}_q\bigr)
  - \lambda_W \, c_W(\tau)
\end{align}
where, for the held-out battery $Q$,
\[
  \mathrm{overlap}(R, F, M_\tau, Q) \;=\;
    \frac{1}{|Q|} \sum_{q \in Q}
    \frac{|\,\mathrm{dia}(R, F, q) \,\cap\, \mathrm{dia}(M_\tau)\,|}
         {|\mathrm{dia}(R, F, q)|}.
\]
$F$ is a flat backend populated with the full conversation text;
$R(q, F)$ is the frozen reader's retrieval given full text;
$\mathrm{dia}(R, F, q)$ is the set of \texttt{dia\_id}s those
retrieved units reference. $\mathrm{dia}(M_\tau)$ is the set of
\texttt{dia\_id}s reachable through entity provenance
(\texttt{Entity.created\_from}) and transition triggers
(\texttt{StateTransition.trigger\_conversation\_id}) in the
budget-pruned post-W KG. The reader $R$ is held fixed across Phase~B.

We default to $w_{\text{ov}} = 0.7$, $w_{\text{qa}} = 0.3$,
$\lambda_W = 0.001$, $K_{\max} = 12$. The cost coefficient is small
because the hard budget already does most of the regularising;
$c_W(\tau)$ is reported per-trajectory for diagnostic logging more
than for reward shaping.

\paragraph{Why reader-overlap.} The earlier version of this paper used
\emph{evidence coverage}---the fraction of a question's gold dia\_ids
preserved in the post-W KG---as the dense signal. A subsequent audit
showed coverage is dominated by a single confound: the number of
turns kept correlates with gold coverage at Spearman $\rho \approx
0.98$ on LoCoMo, so any policy that retains more turns trivially
scores higher under coverage, regardless of \emph{which} turns it
retains. Reader-overlap solves this in two ways: the hard budget
removes the keep-more-stuff degree of freedom, and the target set is
defined by what the reader actually retrieves rather than by gold
labels, which sidesteps both the confound and the deployment problem
that real personal-AI data does not ship with per-question evidence
labels. In a controlled study, reader-overlap correlates with gold
coverage at within-turn $\rho \approx 0.61$, which is far below
$0.98$ but, critically, the residual signal it captures \emph{is} the
content-sensitive part of the original.
